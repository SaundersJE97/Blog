...
class SAC(object):
    ...
    def update_parameters(self, memory:ReplayBuffer, batch_size:int, total_num_steps:int):
        ...
        qf1_loss = functional.mse_loss(qf1, next_q_value) # J(Q) = 𝔼(s_t, a_t)~D[0.5(Q_1(s_t, a_t) - r(s_t, a_t) - γ(𝔼s_(t+1)~p[V(s_t+1)]))^2]
        qf2_loss = functional.mse_loss(qf2, next_q_value)  # J(Q) = 𝔼(s_t, a_t)~D[0.5(Q_2(s_t, a_t) - r(s_t, a_t) - γ(𝔼s_(t+1)~p[V(s_t+1)]))^2]
        ...
        policy_loss = ((self.alpha * log_pi) - min_qf_pi).mean() # J(π) = 𝔼st∼D,εt∼N[α * logπ(f(εt;st)|st) − Q(st,f(εt;st))]
